...
server <- function(input, output, session) {
  ...
  observeEvent(input$chooseDir, {
    ...
    if (!is.na(dirLocation)) {
      output$currDir <- renderText({
        paste(split_path(dirLocation)[1], "(", split_path(dirLocation)[2], ")")
      })
    }
  })
  ...
  renderTable <- function() {
    ...
    if (arbinCM) {
      output$channels <- renderDataTable(data,
        editable = TRUE, options = list(columnDefs = list(list(visible = FALSE, targets = c(3)))),
    ...
    for (group in groupList) {
      ...
      for (row in 1:nrow(dataSubset)) {
        ...
        for (cycle in cycles) {
          ...
          cycle_facts <<- rbind(cycle_facts, data.frame(
            cycle = i, cell = row, chV = chV, dchV = dchV, avgV = (dchV + chV) / 2,
            dV = chV - dchV, DCap = DCap, raw_DCap = tail(cycle$"Discharge_Capacity(Ah)", 1), raw_CCap = tail(cycle$"Charge_Capacity(Ah)", 1), CCap = CCap, CE = CE * 100, lostCap = CCap - DCap, cellFade = if (i == 1) {
              0
            } else {
              DCap - tail(cycle_facts$DCap, 1)
            },
        ...
        if (is.element("dQdV Plots", input$gAnim)) {
          dQdVplot <- function() {
            ...
            lapply(dataSubsetlist, function(plotdataSubset) {
              p <- plot(plotdataSubset$voltage, plotdataSubset$dQdV,
                main = paste("dQdV Plot for", programName, dataSubset$sheet[row], "Cycle", plotdataSubset$cycle[[1]]), xlab = "Voltage (V)", ylab = "dQdV (Ah/V)",
        ...
        if (is.element("Voltage Profiles", input$gAnim)) {
          vpPlot <- function() {
            ...
            lapply(dataSubsetlist, function(plotdataSubset) {
              p <- plot(plotdataSubset$CC, plotdataSubset$`Voltage(V)`,
                main = paste("Voltage Profile for", programName, dataSubset$sheet[row], "Cycle", plotdataSubset$`Cycle_Index`[[1]]), xlab = ylabel, ylab = "Voltage (V)",
      ...
      DCap <- cycle_facts[c("cycle", "DCap")] %>%
        group_by(cycle) %>%
        summarise_each(mean)
      CE <- cycle_facts[c("cycle", "CE")] %>%
        group_by(cycle) %>%
        summarise_each(mean)
      capSEs <- cycle_facts[c("cycle", "DCap")] %>%
        group_by(cycle) %>%
        summarise_each(se)
      ceSEs <- cycle_facts[c("cycle", "CE")] %>%
        group_by(cycle) %>%
        summarise_each(se)
  ...
  output$outputPlot <- renderPlot(
    {
      ...
      if (input$perType == "Within Analysis") {
        ...
        tmp_data$color <<- sapply(tmp_data$cycle, function(x) {
          match(x, input$renderCycles, nomatch = 1)
        })
        tmp_data$symbol <<- sapply(tmp_data$cell, function(x) {
          match(x, cellIndex)
        })
        ...
        switch(input$typeGraph,
          "Charge Voltage" = {
            x <- 0
            chV <- cycle_facts[c("cycle", "chV")] %>%
              group_by(cycle) %>%
              summarise_each(mean)
            chVSE <- cycle_facts[c("cycle", "chV")] %>%
              group_by(cycle) %>%
              summarise_each(se)

            tryCatch(
              {
                comp_chV <- compCycleFacts[c("cycle", "chV")] %>%
                  group_by(cycle) %>%
                  summarise_each(mean)
                comp_chVSE <- compCycleFacts[c("cycle", "chV")] %>%
                  group_by(cycle) %>%
                  summarise_each(se)
                ...
                tmp_data$color <<- sapply(tmp_data$cell, function(x) {
                  match(x, c(1, 2))
                })
          ...
          "Discharge Voltage" = {
            x <- 0
            dchV <- cycle_facts[c("cycle", "dchV")] %>%
              group_by(cycle) %>%
              summarise_each(mean)
            dchVSE <- cycle_facts[c("cycle", "dchV")] %>%
              group_by(cycle) %>%
              summarise_each(se)

            tryCatch(
              {
                comp_dchV <- compCycleFacts[c("cycle", "dchV")] %>%
                  group_by(cycle) %>%
                  summarise_each(mean)
                comp_dchVSE <- compCycleFacts[c("cycle", "dchV")] %>%
                  group_by(cycle) %>%
                  summarise_each(se)
          ...
          "Average Voltage" = {
            avgV <- cycle_facts[c("cycle", "avgV")] %>%
              group_by(cycle) %>%
              summarise_each(mean)
            avgVSE <- cycle_facts[c("cycle", "avgV")] %>%
              group_by(cycle) %>%
              summarise_each(se)

            tryCatch(
              {
                comp_avgV <- compCycleFacts[c("cycle", "avgV")] %>%
                  group_by(cycle) %>%
                  summarise_each(mean)
                comp_avgVSE <- compCycleFacts[c("cycle", "avgV")] %>%
                  group_by(cycle) %>%
                  summarise_each(se)
          ...
          "Delta Voltage" = {
            dV <- cycle_facts[c("cycle", "dV")] %>%
              group_by(cycle) %>%
              summarise_each(mean)
            dVSE <- cycle_facts[c("cycle", "dV")] %>%
              group_by(cycle) %>%
              summarise_each(se)

            tryCatch(
              {
                comp_dV <- compCycleFacts[c("cycle", "dV")] %>%
                  group_by(cycle) %>%
                  summarise_each(mean)
                comp_dVSE <- compCycleFacts[c("cycle", "dV")] %>%
                  group_by(cycle) %>%
                  summarise_each(se)
          ...
          "Discharge Capacity" = {
            DCap <- cycle_facts[c("cycle", "DCap")] %>%
              group_by(cycle) %>%
              summarise_each(mean)
            DCapSE <- cycle_facts[c("cycle", "DCap")] %>%
              group_by(cycle) %>%
              summarise_each(se)

            tryCatch(
              {
                comp_DCap <- compCycleFacts[c("cycle", "DCap")] %>%
                  group_by(cycle) %>%
                  summarise_each(mean)
                comp_DCapSE <- compCycleFacts[c("cycle", "DCap")] %>%
                  group_by(cycle) %>%
                  summarise_each(se)
          ...
          "Charge Capacity" = {
            CCap <- cycle_facts[c("cycle", "CCap")] %>%
              group_by(cycle) %>%
              summarise_each(mean)
            CCapSE <- cycle_facts[c("cycle", "CCap")] %>%
              group_by(cycle) %>%
              summarise_each(se)

            tryCatch(
              {
                comp_CCap <- compCycleFacts[c("cycle", "CCap")] %>%
                  group_by(cycle) %>%
                  summarise_each(mean)
                comp_CCapSE <- compCycleFacts[c("cycle", "CCap")] %>%
                  group_by(cycle) %>%
                  summarise_each(se)
          ...
          "Capacity Loss" = {
            lostCap <- cycle_facts[c("cycle", "lostCap")] %>%
              group_by(cycle) %>%
              summarise_each(mean)
            lostCapSE <- cycle_facts[c("cycle", "lostCap")] %>%
              group_by(cycle) %>%
              summarise_each(se)

            tryCatch(
              {
                comp_lostCap <- compCycleFacts[c("cycle", "lostCap")] %>%
                  group_by(cycle) %>%
                  summarise_each(mean)
                comp_lostCapSE <- compCycleFacts[c("cycle", "lostCap")] %>%
                  group_by(cycle) %>%
                  summarise_each(se)
      ...
      tryCatch(
        {
          if (input$plotStyle == "o" | input$plotStyle == "p" | "se" %in% colnames(tmp_data)) {
            ...
            if (input$typeGraph %in% c("dQdV Graphs", "Voltage Profiles", "Voltage vs. Time")) {
              legend("topright", legend = c(sort(as.numeric(input$renderCycles)), if ("se" %in% colnames(tmp_data)) {
                c(input$dirName, compName)
              }), col = c(unique(tmp_data$color), rep("black", length(input$cells))), lty = c(rep(19, length(unique(tmp_data$color))), 1:length(input$cells)), title = "Cycle", ncol = 2)
            } else {
              legend("topright", legend = c(sort(as.numeric(input$renderCycles)), input$cells, if ("se" %in% colnames(tmp_data)) {
                c(input$dirName, compName)
              }), col = c(unique(tmp_data$color), rep("black", length(input$cells))), lty = c(1:length(input$cells)), title = "Cycle", ncol = 2)
            }
          }
        },
        error = function(cond) {
  ...
  observeEvent(input$gGraphs, {
    ...
    choices <- c(
      "dQdV Graphs", "Voltage Profiles", "Voltage vs. Time", "Discharge Capacity", "Discharge Areal Capacity",
  ...
  observeEvent(input$plot_click, {
    output$hoverCoordx <- renderText({
      input$plot_click$x
    })
    output$hoverCoordy <- renderText({
      input$plot_click$y
    })
  })
  ...
  observeEvent(input$perType, {
    if (input$perType == "Within Analysis") {
      updateRadioButtons(session, "typeGraph", choices = c(
